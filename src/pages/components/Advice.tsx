...
const Advice: React.FC<ScoreProps> = ({ score, advice }) => {
  return (
    <div>
      {score > 0 && score <= 5 && (
        <div className="bg-white border border-gray-300 p-3  rounded-md w-full max-w-[900px]">
          ...
          <li>
            Based on changes in HAM-D scores over time, adjust treatments (e.g.,
            medications, psychotherapy) to optimize outcomes.
          </li>
          <li>
      ...
      {advice ? (
        <div className="bg-white border border-gray-300 p-3  rounded-md w-full max-w-[900px] text-black">
          <p className="text-2xl font-semibold">Advice</p>
          <p>{advice}</p>
        </div>
      ) : null}
